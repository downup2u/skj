...
let rr = require('./shop/rechargerecord.js');
            ...
            if(actiondata.paytype === 'weixin' || actiondata.paytype === 'alipay'){
                pay.getpayresultstring(actiondata.paytype,actiondata.orderdoc,(err,result)=>{
                  if(!err){
                    ...
                    socket.emit('common_err', {errmsg:err.message,type:'getpaysign'});
                  }
                ...
                //检查用户余额是否足够
                  ...
                  userModel.findOne({_id:ctx.userid},(err,targetuser)=>{
                    ...
                    rr.paywithleftbalance(socket,ctx,orderinfo,targetuser,(err,result)=>{
                      if(!err){
                          socket.emit('getpaysign_result', result);
                          socket.emit('serverpush_orderinfo',result);
                      }
                      else{
                          socket.emit('common_err', {errmsg:err,type:'getpaysign'});
                      }
                    });
                  });
              }
