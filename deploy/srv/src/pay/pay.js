...
let getpayresultstring = (paytype,orderDoc,resultcallback)=>{
      ...
      if(paytype == 'alipay'){
        ...
        let getresult = ()=>{
          console.log("keyfilename:"+payobj.keyfilename);
          fs.readFile(payobj.keyfilename, (err, data) => {
              if(err) {
                  console.log('读取支付宝私钥出现错误');
                  resultcallback(err,data);
              } else {
                  var signer = crypto.createSign('RSA-SHA1');
                  signer.update(strcontent,'utf8');
                  var sign = signer.sign(data, "base64");
      			      console.log("第一次sign:" + sign);
                  sign = encodeURIComponent(sign);
      			      console.log("第二次sign:" + sign);
                  retdata = strcontent +
                      '&sign_type="RSA"' +
                      '&sign="' + sign + '"';
                  console.log(retdata);
                  resultcallback(err,retdata);
              }
            });
        };
